...
public class PercentEscaper extends UnicodeEscaper {
   ...
   */
  public PercentEscaper (final String safeChars, final boolean bPlusForSpace) {
    // Avoid any misunderstandings about the behavior of this escaper
    if (safeChars.matches (".*[0-9A-Za-z].*")) {
      throw new IllegalArgumentException ("Alphanumeric characters are always 'safe' and should not be "
                                          + "explicitly specified");
    }
    // Avoid ambiguous parameters. Safe characters are never modified so if
    // space is a safe character then setting plusForSpace is meaningless.
    if (bPlusForSpace && safeChars.contains (" ")) {
      throw new IllegalArgumentException ("plusForSpace cannot be specified when space is a 'safe' character");
    }
    if (safeChars.contains ("%")) {
      throw new IllegalArgumentException ("The '%' character cannot be specified as 'safe'");
    }
    this.plusForSpace = bPlusForSpace;
  ...
  private static boolean [] createSafeOctets (final String safeChars) {
    int maxChar = 'z';
    final char [] safeCharArray = safeChars.toCharArray ();
    for (final char c : safeCharArray) {
   ...
   */
  @Override
  protected int nextEscapeIndex (final CharSequence csq, final int pindex, final int end) {
    int index = pindex;
    for (; index < end; index++) {
   ...
   */
  @Override
  protected char [] escape (final int pcp) {
    int cp = pcp;
          ...
          if (cp <= 0x7ff) {
            ...
            if (cp <= 0xffff) {
              ...
              if (cp <= 0x10ffff) {
                ...
                // If this ever happens it is due to bug in UnicodeEscaper, not
                // bad input.
