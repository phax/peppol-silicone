...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="T15">
  <rule context="$Supplier_Party">
    ...
    <assert flag="fatal" test="$NOGOV-T15-R014">[NOGOV-T15-R014]-Country code for the supplier address MUST be provided according to EHF.</assert>
  </rule>
  <rule context="$Payment_Means">
    <assert flag="warning" test="$NOGOV-T15-R011">[NOGOV-T15-R011]-PayeeFinancialAccount SHOULD be provided  according EHF.</assert>
  ...
  <rule context="$Order_Reference">
    <assert flag="warning" test="$NOGOV-T15-R013">[NOGOV-T15-R013]-An association to Order Reference SHOULD be provided according to EHF.</assert>
  </rule>
  <rule context="$Item">
  ...
  <rule context="$Invoice">
    <assert flag="warning" test="$NOGOV-T15-R005">[NOGOV-T15-R005]-ContractDocumentReference SHOULD be provided according to EHF.</assert>
  </rule>
  <rule context="$Customer_Party">
    <assert flag="warning" test="$NOGOV-T15-R006">[NOGOV-T15-R006]-A customer number for AccountingCustomerParty SHOULD be provided according to EHF.</assert>
    <assert flag="fatal" test="$NOGOV-T15-R007">[NOGOV-T15-R007]-A contact reference identifier MUST be provided for AccountingCustomerParty according to EHF.</assert>
    <assert flag="fatal" test="$NOGOV-T15-R009">[NOGOV-T15-R009]-PartyLegalEntity for AccountingCustomerParty MUST be provided according to EHF.</assert>
  </rule>
</pattern>
