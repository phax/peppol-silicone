...
public final class Validator implements CertificateValidator {
   ...
   */
  private static final String TRUSTSTORE_PATH = "peppol.servertrust";

  /**
   * Server truststore password.
   */
  private static final String TRUSTSTORE_PASS = "peppol.password";

  /**
   * Server truststore alias.
   */
  private static final String TRUSTORE_ALIAS = "peppol.alias";

  /**
   * Configuration for certificates.
   */
  private static final ConfigFile s_aConf = new ConfigFile ("configOCSP.properties");
  ...
  public static final EValidity certificateValidate (final X509Certificate xc, @Nonnull final String sFilePath) {
    try {
      // Load keystore
      final String sKeyStorePassword = s_aConf.getString (TRUSTSTORE_PASS);
      final KeyStore aKeyStore = KeyStoreUtils.loadKeyStoreFromFile (sFilePath, sKeyStorePassword);

      // Get certificate from alias
      final X509Certificate rootcert = (X509Certificate) aKeyStore.getCertificate (s_aConf.getString (TRUSTORE_ALIAS));
