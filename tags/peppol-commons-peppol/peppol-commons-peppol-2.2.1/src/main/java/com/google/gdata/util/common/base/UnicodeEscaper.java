...
public abstract class UnicodeEscaper implements Escaper {
  ...
  protected int nextEscapeIndex (final CharSequence csq, final int start, final int end) {
    int index = start;
    while (index < end) {
      final int cp = codePointAt (csq, index, end);
  ...
  public String escape (final String string) {
    ...
    final int index = nextEscapeIndex (string, 0, end);
   ...
   *         if invalid surrogate characters are encountered
   */
  protected final String escapeSlow (final String s, final int pindex) {
    int index = pindex;
    ...
    while (index < end) {
      ...
      if (escaped != null) {
        ...
        if (dest.length < sizeNeeded) {
          final int destLength = sizeNeeded + (end - index) + DEST_PAD;
    ...
    final int charsSkipped = end - unescapedChunkStart;
    if (charsSkipped > 0) {
      final int endIndex = destIndex + charsSkipped;
   ...
   */
  protected static final int codePointAt (final CharSequence seq, final int pindex, final int end) {
    int index = pindex;
    if (index < end) {
