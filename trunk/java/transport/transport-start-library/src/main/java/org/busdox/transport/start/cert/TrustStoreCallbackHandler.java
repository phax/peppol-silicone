...
public final class TrustStoreCallbackHandler implements CallbackHandler {
  private static final Logger log = LoggerFactory.getLogger (TrustStoreCallbackHandler.class);
  ...
  public void handle (final Callback [] aCallbacks) throws IOException, UnsupportedCallbackException {
    ...
    final String sTrustStorePath = aConfigFile.getString ("org.busdox.transport.start.server.truststore.path");
    final String sTrustStorePassword = aConfigFile.getString ("org.busdox.transport.start.server.truststore.password");
    final String sTrustStoreAlias = aConfigFile.getString ("org.busdox.transport.start.server.truststore.alias");
    final char [] aTrustStoreAliasPassword = aConfigFile.getCharArray ("org.busdox.transport.start.server.truststore.aliaspassword");

    log.info ("Loading trust store from" + sTrustStorePath);
    KeyStore aKeyStore;
    try {
    ...
    for (final Callback aCallback : aCallbacks) {
      if (aCallback instanceof KeyStoreCallback) {
        try {
          ...
          log.error ("Failed to set keystore", ex);
        }
      }
      else
        if (aCallback instanceof PrivateKeyCallback) {
          try {
            ...
            log.error ("Failed to set private key", ex);
          }
        }
    }
  }
