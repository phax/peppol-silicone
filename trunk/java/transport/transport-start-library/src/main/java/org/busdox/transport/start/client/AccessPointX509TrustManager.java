...
public class AccessPointX509TrustManager implements X509TrustManager {
  ...
  AccessPointX509TrustManager (@Nullable final Set <String> acceptedCommonNames,
                               @Nullable final X509Certificate acceptedRootCertificate) {
  ...
  private void checkIssuer (final X509Certificate [] chain) {
    ...
    if (!chain[0].getIssuerX500Principal ()
                 .toString ()
                 .equals (m_aRootCertificate.getIssuerX500Principal ().toString ())) {
  ...
  private void checkPrincipal (final X509Certificate [] chain) throws CertificateException {
    if (m_aCommonNames != null) {
      ...
      final String sSubject = chain[0].getSubjectX500Principal ().toString ();
      final StringTokenizer st = new StringTokenizer (sSubject, ",");
      while (st.hasMoreTokens ()) {
        final String tok = st.nextToken ();
        final int x = tok.indexOf ("CN=");
        if (x >= 0) {
          final String curCN = tok.substring (x + 3);
          if (m_aCommonNames.contains (curCN)) {
            commonNameOK = true;
      ...
      if (!commonNameOK) {
        log.warn ("No accepted issuer: " + sSubject);
