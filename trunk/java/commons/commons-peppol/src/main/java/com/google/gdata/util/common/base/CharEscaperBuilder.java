...
public class CharEscaperBuilder {
  ...
  private static class CharArrayDecorator extends CharEscaper {
    ...
    public CharArrayDecorator (final char [][] replacements) {
      this.m_aReplacements = replacements;
      this.m_nReplaceLength = replacements.length;
    }
    ...
    public String escape (final String s) {
      ...
      for (int index = 0; index < slen; index++) {
        ...
        if (c < m_aReplacements.length && m_aReplacements[c] != null) {
    ...
    protected char [] escape (final char c) {
      return c < m_nReplaceLength ? m_aReplacements[c] : null;
    }
  }
   ...
   */
  public char [][] toArray () {
    final char [][] result = new char [max + 1] [];
    for (final Map.Entry <Character, String> entry : map.entrySet ()) {
